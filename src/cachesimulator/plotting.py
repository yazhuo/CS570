...
input_dir = "/research/yazhuo/CS570/Output/"
...
def plot_cdn_mrcs(single_mrc, partition_mrc, max_cache, step):
    ...
    # print(s_mrc)
    # print(p_mrc)
    w = open('kmedoids_mrc_1384_fs', 'w')
    for i in range(len(p_mrc)):
        w.write(str(p_mrc[i]) + '\n')
    w.close()
...
def plotting_from_file():

    file1 = "kmedoids_mrc_1448_no"
    file2 = "kmedoids_mrc_1448_fs"
    ...
    plt.plot(x_axis, m1, label="No Feature Selection")
    plt.plot(x_axis, m2, label="Feature Selection")
...
def cache_variable_mrc(max_cache, step, K):
    file_loc = input_dir + MS[m_index] + DS[d_index] + 'rd_single'
    ...
    for i in range(K):
        print(i)
        file_loc = input_dir + MS[m_index] + DS[d_index] + 'rd_cluster_' + str(i)
...
def read_customer_rds(customer_id, cluster_id):
    ...
    file_loc1 = input_dir + MS[m_index] + DS[d_index] + "rd_cluster_8_" + str(customer_id)
...
if __name__ == "__main__":
    ...
    # python3 plotting.py 0 0 6 kmeans dc_1448
    # python3 plotting.py 1 0 6 Kmedoids dc_1448
    # python3 plotting.py 1 1 4 Kmedoids dc_1448
    # python3 plotting.py 1 2 6 Kmedoids dc_1384
